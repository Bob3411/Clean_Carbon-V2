	...
	public static void partIndv(int fml, double carbonCol) {
		...
		if (carbonTL > 7000) {
			System.out.printf("A junção de produção individual e coletiva é : %.2f \n"
					+ "\u001B[31mvocê está produzindo muito carbono, procure diminuir rapidamente a produção de carbono.\n",
					carbonTL);
		} else if ((carbonTL > 3000) && (carbonTL <= 6999)) {
			System.out.printf(
					"A junção de produção individual e coletiva é : %.2f \n"
							+ "\u001B[34mvocê está produzindo carbono de forma moderada, tome cuidado com a produção de carbono.\n",
					carbonTL);
		} else {
			System.out.printf("A junção de produção individual e coletiva é : %.2f \n"
					+ "\u001B[32mvocê está produzindo pouquissíma qunatidade de carbono.\n" + "Parabéns, o planeta te agradece.\n",
					carbonTL);
		}
        int[] valores = {EmissaoIndividual.QueimaCombustivel(consuOnibus, consuMetro), 5, 2}; // Valores para cada barra
